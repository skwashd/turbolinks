...
/**
 * @file
 * Contains \Drupal\turbolinks\TurbolinksPageState.
 */
...
class TurbolinksPageState {
  ...
  /**
   * Constructs a new TurbolinksPageState instance.
   *
  ...
  /**
   * Builds the Turbolinks page state for the given response cacheability.
   *
   * @param \Drupal\Core\Cache\CacheableMetadata $cacheability
   *   Response cacheability metadata (or at least the cacheability metadata of
   *   all rendered parts of the page that are eligible for Turbolinks-based
   *   updating).
   *
   * @return array
   *   The Turbolinks page state to be used in #attached[drupalSettings].
   */
  public function build(CacheableMetadata $cacheability) {
    // For now, the Turbolinks page state only consists of the context hashes.
    return $this->getSensitiveContextHashes($cacheability->getCacheContexts());
  }

  /**
   * Reads the Turbolinks page state from the current request.
   *
   * @param \Symfony\Component\HttpFoundation\Request $request
   *   The current request.
   *
   * @return array
   *   The Turbolinks page state.
   *
   * @throws \Symfony\Component\HttpKernel\Exception\HttpException
   *   When the Turbolinks page state is missing.
   */
  protected function read(Request $request) {
    if (!$request->query->get('turbolinks_page_state')) {
      throw new HttpException('Turbolinks page state is missing.');
    }

    $turbolinks_page_state = $request->query->get('turbolinks_page_state');
    return [
      // @see build()
      'cache_contexts' => $turbolinks_page_state,
    ];
  }
  ...
  /**
   * Indicates whether Turbolinks is sensitive to changes in this cache context.
   *
   * Turbolinks is sensitive to only the 'url' (and 'url.*') cache context and
  ...
  /**
   * Gets the context hashes that Turbolinks is sensitive to.
   *
